...
interface Props {
  /** Token from the ?t= query param (resolved server-side). */
  token?: string;
}

export function PublicSnapshotClient({ token: tokenProp }: Props) {
  const [state, setState] = React.useState<PublicSnapshotState>({
    status: "loading",
  });

  React.useEffect(() => {
    // Prefer query-param token; fall back to hash for old share links.
    const token = tokenProp?.trim() || window.location.hash.slice(1).trim();
    if (!token) {
      setState({
        status: "error",
