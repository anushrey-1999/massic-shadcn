...
export function AgentHistorySidebar({
  ...
  return (
    <aside className="relative flex h-full w-full flex-col border-r border-border bg-sidebar text-sidebar-foreground overflow-hidden">

      {/* Collapsed icon rail */}
      <div className={cn(
        "absolute inset-0 flex flex-col items-center py-3 gap-1 transition-opacity duration-200",
        isCollapsed ? "opacity-100 pointer-events-auto delay-150" : "opacity-0 pointer-events-none"
      )}>
        <Button type="button" variant="ghost" size="icon-sm" onClick={onExpand} aria-label="Expand sidebar" className="h-8 w-8 text-muted-foreground">
        ...
        <div className="mt-2 flex flex-col items-center gap-1">
          {primaryNavItems.map((item) => {
            ...
            return (
              <Button key={item.id} type="button" variant="ghost" size="icon-sm" onClick={item.onClick} aria-label={item.label}
                className={cn("h-8 w-8 text-muted-foreground hover:bg-general-primary/8", isActive && "bg-general-primary/8")}>
      ...
      <div className={cn(
          ...
          {primaryNavItems.map((item) => {
            ...
            return (
              <button key={item.id} type="button" onClick={item.onClick}
                className={cn(
                  "flex h-8 cursor-pointer items-center gap-2.5 rounded-md px-2 text-sm text-sidebar-foreground transition-colors hover:bg-general-primary/8",
                ...
                >
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button type="button" variant="ghost" size="icon-sm" onClick={(e) => e.stopPropagation()} aria-label="Chat options"
        ...
        <div className="border-t border-border px-2 py-2">
          <div className="flex items-center gap-2 rounded-md px-1.5 py-1.5 hover:bg-sidebar-accent/60">
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-general-primary/15 text-xs font-semibold text-general-primary">M</div>
