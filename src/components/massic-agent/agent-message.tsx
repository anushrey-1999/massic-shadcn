...
export function AgentMessageView({ message, streaming, activeResource, onOpenPlan }: { message: AgentMessage; streaming: boolean; activeResource: ResourceRef | null; onOpenPlan: (resource: ResourceRef) => void }) {
  ...
  return <article className="flex min-w-0 flex-col gap-3">
    ...
    </details> : streaming && <div className="flex w-fit items-center gap-2 px-2 py-1.5 text-xs text-muted-foreground" role="status"><MassicLoader size={18} animate /><span>Thinking…</span></div>}
