...
import { Check, Copy, Loader2, ChevronDown, CircleAlert, ExternalLink } from "lucide-react";
...
export function AgentMessageView({ message, streaming, onOpenPlan }: { message: AgentMessage; streaming: boolean; onOpenPlan: (resource: ResourceRef) => void }) {
  ...
  return <article className="flex min-w-0 flex-col gap-3">
    ...
    {message.widgetParts?.map(part => <button key={`${part.resource.type}:${part.resource.id}`} onClick={() => onOpenPlan(part.resource)} className="flex w-full cursor-pointer items-center gap-3 rounded-xl border border-border bg-card px-4 py-3 text-left shadow-sm transition-[background-color,border-color,box-shadow] hover:border-general-primary/20 hover:bg-muted/30 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-general-primary/30"><span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-general-primary/10 text-general-primary"><SurfaceIcon surface={resourceSurface(part.resource.type)} className="size-4" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-medium">{SURFACES[resourceSurface(part.resource.type)].label} plan #{part.resource.id}</span><span className="text-xs text-muted-foreground">View plan and select items to refine</span></span><span className="text-xs font-medium text-general-primary">Open</span></button>)}
