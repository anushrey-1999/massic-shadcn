...
export const AgentMessageView = memo(function AgentMessageView({ message, streaming, activeResource, onOpenPlan }: { message: AgentMessage; streaming: boolean; activeResource: ResourceRef | null; onOpenPlan: (resource: ResourceRef) => void }) {
  ...
  return <article className="flex min-w-0 flex-col gap-3">
    {(activity.length > 0 || streaming) && <Collapsible className="group w-fit max-w-full text-sm"><CollapsibleTrigger disabled={!activity.length} className="disabled:cursor-default disabled:hover:bg-transparent disabled:hover:shadow-none flex w-fit max-w-full cursor-pointer list-none items-center gap-2 rounded-md px-2 py-1.5 text-xs text-muted-foreground transition-[background-color,box-shadow,color] hover:bg-muted hover:text-foreground hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-general-primary/30">{streaming ? <MassicLoader size={18} animate={false} /> : <Check className="h-3.5 w-3.5" />}<span className={cn("truncate", streaming && styles.thinkingShimmer)} role="status">{streaming ? latest?.label ?? "Thinking…" : "Thinking"}</span>{activity.length > 0 && <ChevronDown className="h-3.5 w-3.5 shrink-0 transition-transform duration-200 group-data-[state=open]:rotate-180 motion-reduce:transition-none" aria-hidden="true" />}</CollapsibleTrigger><CollapsibleContent className={styles.thinkingContent}>
    ...
    {message.widgetParts?.map(part => {
      ...
      return <button key={`${part.resource.type}:${part.resource.id}`} type="button" aria-pressed={isOpen} data-state={isOpen ? "open" : "closed"} onClick={() => onOpenPlan(part.resource)} className={cn("group/plan flex w-full cursor-pointer items-center gap-3 rounded-xl border bg-card px-4 py-3 text-left shadow-sm transition-[background-color,border-color,box-shadow] [transition-duration:160ms] ease-out hover:border-general-primary/25 hover:bg-general-primary/5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-general-primary/30 motion-reduce:transition-none", isOpen ? "border-general-primary/30 bg-general-primary/5 shadow-sm" : "border-border")}><span className={cn("flex size-8 shrink-0 items-center justify-center rounded-lg bg-general-primary/10 text-general-primary transition-colors [transition-duration:160ms] ease-out motion-reduce:transition-none", isOpen && "bg-general-primary/15")}><SurfaceIcon surface={resourceSurface(part.resource.type)} className="size-4" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-medium">{SURFACES[resourceSurface(part.resource.type)].label} plan #{part.resource.id}</span><span className="text-xs text-muted-foreground">View plan and select items to refine</span></span><span className={cn("flex size-8 shrink-0 items-center justify-center rounded-full transition-[background-color,color,transform] [transition-duration:160ms] ease-out motion-reduce:transition-none", isOpen ? "bg-general-primary/10 text-general-primary" : "text-muted-foreground group-hover/plan:translate-x-0.5 group-hover/plan:bg-general-primary/10 group-hover/plan:text-general-primary")} aria-hidden="true"><ChevronRight className="size-4" /></span></button>;
    })}
