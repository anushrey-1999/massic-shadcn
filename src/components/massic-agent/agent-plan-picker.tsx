...
export function AgentPlanPicker({ businessId, type, open, onOpenChange, onPick }: { businessId: string; type: ResourceType; open: boolean; onOpenChange: (open: boolean) => void; onPick: (resource: ResourceRef) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Open {type === "webpage_plan" ? "webpage" : "social"} plan</DialogTitle><DialogDescription>Choose a plan to view, refine, or activate.</DialogDescription></DialogHeader>
