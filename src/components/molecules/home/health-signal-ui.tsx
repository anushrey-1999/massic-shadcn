...
const HEALTH_TOOLTIP_NARRATIVE: Record<
  "green" | "amber" | "red",
  {
    strongSignal: { subtitle: string; footer: string };
    thinData: { subtitle: string; footer: string };
  }
> = {
  green: {
    strongSignal: {
...
export function HealthMetricComparisonRow({
  icon,
  label,
  recent,
  baseline,
...
export function HealthDailyMetrics({ status }: { status: HealthStatusRow }) {
  const hasMetrics =
    status.recent_leads != null || status.recent_traffic != null;
