...
/** The original pixel M loader, retained for legacy and brand-mark uses. */
export function MassicMLoader({ className, size = 36, animate = true }: Props) {
  const cell = size / 6;
  const gap = cell * 0.18;
  const box = cell - gap;
        ...
        return (
          <span
            ...
            style={{
              ...
              top: row * cell + gap / 2,
              left: col * cell + gap / 2,
              animation: animate ? `pulse ${duration}s linear infinite` : undefined,
